// Action types related with the buttons
export const CHANGE_BUTTON_NAME = 'CHANGE_BUTTON_NAME';
export const CHANGE_BUTTON_NAME_SUCCESS = 'CHANGE_BUTTON_NAME_SUCCESS';
export const CHANGE_BUTTON_NAME_FAILED = 'CHANGE_BUTTON_NAME_FAILED';
// Action types related with the widgets
export const FETCH_WIDGETS_REQUEST = 'FETCH_WIDGETS_REQUEST';
export const FETCH_WIDGETS_SUCCESS = 'FETCH_WIDGETS_SUCCESS';
export const FETCH_WIDGETS_FAILURE = 'FETCH_WIDGETS_FAILURE';
export const UNIT_WIDGET_EDITED = 'UNIT_WIDGET_EDITED';
export const UNIT_WIDGET_SUCCESS = 'UNIT_WIDGET_SUCCESS';
export const UNIT_WIDGET_FAILED = 'UNIT_WIDGET_FAILED';
export const ADD_WIDGET = 'ADD_WIDGET';
export const ADD_WIDGET_SUCCESS = 'ADD_WIDGET_SUCCESS';
export const ADD_WIDGET_FAILED = 'ADD_WIDGET_FAILED';
export const CHANGE_WIDGET_NAME = 'CHANGE_WIDGET_NAME';
export const CHANGE_WIDGET_NAME_SUCCESS = 'CHANGE_WIDGET_NAME_SUCCESS';
export const CHANGE_WIDGET_NAME_FAILED = 'CHANGE_WIDGET_NAME_FAILED';
export const REMOVE_WIDGET_ID = 'REMOVE_WIDGET_ID';
export const REMOVE_WIDGET = 'REMOVE_WIDGET';
export const REMOVE_WIDGET_SUCCESS = 'REMOVE_WIDGET_SUCCESS';
export const REMOVE_WIDGET_FAILED = 'REMOVE_WIDGET_FAILED';
// Action types related with the rooms
export const ROOM_ADDED = 'ROOM_ADDED';
export const ROOM_ADDED_SUCCESS = 'ROOM_ADDED_SUCCESS';
export const ROOM_ADDED_FAILED = 'ROOM_ADDED_FAILED';
export const ROOM_NAME_CHANGED = 'ROOM_NAME_CHANGED';
export const ROOM_NAME_CHANGED_SUCCESS = 'ROOM_NAME_CHANGED_SUCCESS';
export const ROOM_NAME_CHANGED_FAILED = 'ROOM_NAME_CHANGED_FAILED';
export const ROOM_CHANGED = 'ROOM_CHANGED';
export const CLEAR_ALL_WIDGETS = 'CLEAR_ALL_WIDGETS';
export const TOGGLE_EDIT_ROOM = 'TOGGLE_EDIT_ROOM';
export const REMOVE_ROOM_ID = 'REMOVE_ROOM_ID';
export const REMOVE_ROOM = 'REMOVE_ROOM';
export const REMOVE_ROOM_SUCCESS = 'REMOVE_ROOM_SUCCESS';
export const REMOVE_ROOM_FAILED = 'REMOVE_ROOM_FAILED';
// Action types related with the modals
export const ADD_ROOM_MODAL_OPENED = 'ADD_ROOM_MODAL_OPENED';
export const ADD_ROOM_MODAL_CLOSED = 'ADD_ROOM_MODAL_CLOSED';
export const REMOVE_ROOM_MODAL_OPENED = 'REMOVE_ROOM_MODAL_OPENED';
export const REMOVE_ROOM_MODAL_CLOSED = 'REMOVE_ROOM_MODAL_CLOSED';
export const ADD_WIDGET_MODAL_OPENED = 'ADD_WIDGET_MODAL_OPENED';
export const ADD_WIDGET_MODAL_CLOSED = 'ADD_WIDGET_MODAL_CLOSED';
export const REMOVE_WIDGET_MODAL_OPENED = 'REMOVE_WIDGET_MODAL_OPENED';
export const REMOVE_WIDGET_MODAL_CLOSED = 'REMOVE_WIDGET_MODAL_CLOSED';
// Action types related with the user
export const FETCH_USER_INFO = 'FETCH_USER_INFO';
export const UPDATE_USER_INFO = 'UPDATE_USER_INFO';
export const USER_UPDATE_SUCCESS = 'USER_UPDATE_SUCCESS';
export const USER_UPDATE_FAILURE = 'USER_UPDATE_FAILURE';