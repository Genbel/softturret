...
import { paginateRoom } from 'actions/dashboard/roomsActions';
...
class RoomPagination extends Component {
    ...
    changeActualRoom(action){
        const { paginateRoom, actualRoom } = this.props;
        const futurePage = action === 'NEXT'? actualRoom + 1: actualRoom -1;
        paginateRoom(futurePage);
    }
...
RoomPagination.PropTypes = {
    ...
    paginateRoom: React.PropTypes.func.isRequired
};

const mapDispatchToProps = (dispatch) => bindActionCreators({ paginateRoom }, dispatch);
