...
import { getRoomStateObject, getFullObjectOfTheState, getPaginationState, getRoomStateAfterDisconnectTheWidget, getNewRoomState } from '../../../../mockElements/dashboard/states';
...
describe('#dashboard-reducers: room reducer (Reducer, roomReducer.js)', () => {
    describe('-> creating new state of the room reducer', () => {
        ...
        it('should handle ROOM_CHANGED action', () => {
            const newState = { byId: {}, actual: 1, pagination: [], editMode: false, sendingRequest: false, removedRoomId: null };
            expect(reducer({}, { type: ROOM_CHANGED, page: 1 })).to.eql(newState);
        });
        it('should handle ROOM_ADDED_SUCCESS action', () => {
            const action = newRoomAction();
            const newState = getNewRoomState();
        ...
        it('should handle TOGGLE_EDIT_ROOM action', () => {
            ...
            const state = { byId: {}, actual: 1, pagination: [], editMode: false, removedRoomId: null, sendingRequest: false };
            const newState = { byId: {}, actual: 1, pagination: [], editMode: true, removedRoomId: null, sendingRequest: false };
            expect(reducer(state, action)).to.eql(newState);
